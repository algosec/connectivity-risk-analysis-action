  ...
  async check(
    ...
    const asyncIterable = async (iterable, action) => {
      for (const [index, value] of iterable?.entries()) {
        const output = await action({ runFolder: value?.replace(workDir, "")?.substring(1, value?.length - 1), workDir, path: value }, this.vcs);
        const file: RiskAnalysisFile = {
          uuid: uuid.v4(),
          folder: value?.replace(workDir, "")?.substring(1, value?.length - 1),
          output,
        };
